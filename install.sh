...
if [ "$MINIMAL" = false ]; then
  ...
  if command -v ollama &>/dev/null; then
    ...
    read -p "  Pull recommended LLM model (nemotron-3-nano:4b)? [y/N] " ans
    if [[ "$ans" =~ ^[Yy] ]]; then
      info "Pulling nemotron-3-nano:4b (this may take a while)..."
    ...
    info "  Then: ollama serve & && ollama pull nemotron-3-nano:4b"
  fi
fi
