...
impl RuntimeEngine {
    ...
    async fn run_live_or_paper(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // Check for hard stop
                {
                    ...
                    if dd * 100.0 > 15.0 {
                        tracing::error!(
                            "⚠ HARD STOP: Drawdown {:.1}% exceeds limit.",
                            dd * 100.0
                        );
                    }
                }
        ...
        let mut ohlcv_tick_counters: std::collections::HashMap<String, u32> = std::collections::HashMap::new();

        loop {
            match rx.recv().await {
                Ok(event) => {
                    match event {
                        AgentEvent::PriceTick { symbol, price, volume, timestamp, .. } => {
                            if symbol == "HEARTBEAT" {
                            ...
                            tracing::debug!(
                                "⚡ Event: PriceTick {} @ {:.2}", symbol, price
                            );
                            ...
                            // 1. Update OHLCV history (counter tracks bar rollover at ~1 min intervals)
                            {
                                ...
                                let tick_count = ohlcv_tick_counters.entry(symbol.clone()).or_insert(0u32);
                            ...
                            // 2. Update world model with real price change magnitude
                            {
                                ...
                                let prev_close = hist.get(&symbol)
                                    .and_then(|h| h.iter().rev().nth(1).map(|b| b.close))
                                    .unwrap_or(price);
                                let magnitude = ((price - prev_close) / prev_close.max(0.001)).abs();
                            ...
                            let (_signal, decision_source, executed) = self.make_decision(&symbol, price).await;
                            ...
                            if executed || cycles.load(std::sync::atomic::Ordering::Relaxed) % 10 == 0 {
                                tracing::debug!(
                                    "Decision for {}: source={}, executed={}",
                                    symbol, decision_source, executed
                                );
                            }
    ...
    async fn run_backtest(
        ...
        let data_path = self.mode_config.backtest_data_path
            .clone()
        ...
        let mut bar_count = 0;
        while feed.has_next() {
            let bars = feed.next_bars().await
                .map_err(|e| anyhow::anyhow!("Failed to read next bar: {}", e))?;
            for _bar in bars {
                bar_count += 1;

                // Run the pipeline on each bar
                for sym in &self.symbols {
                    match self.orchestrator.run_full_pipeline(sym).await {
                        Ok(_) => {}
                        Err(e) => {
                            tracing::error!("Pipeline error at bar {} for {}: {}", bar_count, sym, e);
                        }
                    }
                }
            ...
            if bar_count % 100 == 0 {
                tracing::info!("Backtest progress: {:.1}%", feed.progress() * 100.0);
            }
        }

        tracing::info!("Backtest complete: {} bars processed, {} cycles", bar_count, self.cycles_completed);
    ...
    async fn run_validate(
        ...
        tracing::info!("Starting validate mode ({} cycles)", self.mode_config.validate_cycles);

        for cycle in 0..self.mode_config.validate_cycles {
            for sym in &self.symbols {
                match self.orchestrator.run_full_pipeline(sym).await {
                    Ok(_) => {}
                    Err(e) => {
                        tracing::error!("Validation cycle {} for {} error: {}", cycle, sym, e);
                    }
                }
            }
            ...
            tracing::info!(
                "Cycle {}/{} | Mode: {:?} | Conf: {:.2} | Data: {:?} | Acc: {:.2}",
                cycle + 1,
                self.mode_config.validate_cycles,
                introspection.mode,
                introspection.market_model_confidence,
                introspection.data_quality,
                introspection.recent_accuracy,
            );
    ...
    fn build_summary(&self, trades_executed: Option<usize>, cache_hits: usize, ollama_calls: usize) -> RunSummary {
        RunSummary {
    ...
    pub async fn make_decision(
        ...
        if let Some(entry) = cached {
            ...
            if let Some(signal) = self.signal_from_cache(&features, &entry, current_price).await {
                ...
                let result = if is_live {
                    ...
                    self.orchestrator.execution.execute_paper_trade(&signal).await
                        .map(|_| ())
                ...
                match result {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "PolicyCache: execution failed for {}: {}",
                            symbol,
                            e
                        );
    ...
    async fn signal_from_cache(
        ...
        let size = {
            ...
            if stop_distance > 0.0 {
                (risk_amount / stop_distance).max(1.0).min(equity / current_price * 0.1)
            } else {
                10.0
            }
        };
