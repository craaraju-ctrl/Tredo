...
use tredo_core::paper_engine::{
    OrderRequest, OrderType, PaperEngine, PaperEngineConfig, PortfolioSummary, Position,
    RiskCheckResult,
};
    ...
    async fn place_market_order(
        ...
        let market_price = {
            ...
            prices.get(symbol).copied().ok_or_else(|| {
                format!(
                    "No price data for {}. Call update_price_cache() first.",
                    symbol
                )
            })?
        };
...
mod tests {
    ...
    async fn test_monitor_positions_sl_tp() {
        ...
        broker.engine().place_order(request, 50000.0).await.unwrap();
        ...
        assert_eq!(
            ...
            tredo_core::paper_engine::CloseReason::StopLoss
        );
    }
