...
impl StreamingReasoner {
    pub fn new(state: SharedState) -> Self {
        Self { state, llm: None, max_iterations: 5 }
    }

    pub async fn reason(&self, _symbol: &str, _current_price: f64) -> Result<ReasoningOutcome, String> {
        // Placeholder: when llm is None, return a basic reasoning without LLM calls
        Ok(ReasoningOutcome {
            transcript: vec![ReasoningStep {
                step_type: StepType::Observation,
                content: "StreamingReasoner: no LLM configured, using passive observation".to_string(),
                confidence: 0.3,
            ...
            confidence: 0.3,
            iterations_used: 1,
            final_recommendation: "Insufficient reasoning capacity — deferring to fast pipeline".to_string(),
        })
    }

    #[allow(dead_code)]
    async fn synthesize(&self, transcript: &[ReasoningStep]) -> Result<String, String> {
        let summary = transcript.iter()
            .map(|s| format!("{:?}: {}", s.step_type, s.content))
            .collect::<Vec<_>>()
            .join("\n");
        Ok(format!("Final decision based on {} reasoning steps:\n{}", transcript.len(), summary))
    }
}
