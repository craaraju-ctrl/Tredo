...
impl BrokerPluginManager {
    ...
    fn load_builtins(&mut self) {
        ...
        self.plugins.push(BrokerPlugin {
            ...
            description: "US equities & crypto via Alpaca Markets API v2. Supports paper (free) and live.".into(),
            config_schema: vec![
                ConfigField {
                    key: "api_key_id".into(),
    ...
    pub async fn instantiate(
        ...
        match plugin.implementation.as_str() {
            "builtin:paper" => {
                ...
                let mut cfg = PaperEngineConfig::default();
            ...
            "builtin:zerodha" => {
                let api_key = merged
                    .get("api_key")
                    .ok_or_else(|| {
                        "Zerodha: api_key required (set in config or ZERODHA_API_KEY env)".to_string()
                    })?
                    .to_string();
