...
impl PortfolioReasoner {
    pub fn new(state: SharedState) -> Self {
        Self { state, max_sector_exposure: 0.30, max_correlation: 0.85 }
    }
    ...
    pub async fn should_open_new(&self, symbol: &str, direction: TradeDirection) -> Decision {
        ...
        if equity <= 0.0 {
            return Decision::Reject { reason: "Zero equity".to_string() };
        }
        let current_heat: f64 = portfolio.open_positions.iter()
            .map(|p| p.risk_amount).sum::<f64>() / equity;
        ...
        let sector_exposure: f64 = portfolio.open_positions.iter()
            .filter(|p| Self::get_sector(&p.symbol) == sector)
            .map(|p| p.quantity * p.current_price)
            .sum::<f64>() / equity;
        if sector_exposure > self.max_sector_exposure {
            return Decision::Reject {
                reason: format!("{} sector exposure at {:.1}%", sector, sector_exposure * 100.0)
            };
        }
        ...
        for pos in portfolio.open_positions.iter() {
            ...
            {
                return Decision::Reject {
                    reason: format!("High correlation ({:.2}) with existing {}", corr, pos.symbol)
                };
            }
        }

        drop(portfolio);
        Decision::Approve { reason: "Portfolio composition allows this trade".to_string() }
    }
    ...
    fn direction_matches(a: &TradeDirection, b: &TradeDirection) -> bool {
        matches!((a, b), (TradeDirection::Long, TradeDirection::Long) | (TradeDirection::Short, TradeDirection::Short))
    }
