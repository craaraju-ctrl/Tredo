...
impl GoalManager {
    ...
    fn seed_default_goals(&mut self) {
        ...
        self.goals.push(Goal {
            ...
            description: "Accumulate at least 20 high-quality trade episodes per regime"
                .to_string(),
            goal_type: GoalType::Learning {
                topic: "regime_adaptation".to_string(),
            },
            priority: 5,
            status: GoalStatus::Active,
            current_progress: 0.0,
    ...
    pub async fn update_progress(&mut self) {
        for goal in self.goals.iter_mut() {
            ...
            let new_progress = match &goal.goal_type {
                ...
                GoalType::Capability { feature: _ } => 0.0,
            ...
            if (new_progress - goal.current_progress).abs() > 0.01 {
                ...
                self.history.push(GoalEvent {
                    goal_id: goal.id.clone(),
                    event_type: GoalEventType::ProgressMade {
                        from: old,
                        to: new_progress,
                    },
                    timestamp: chrono::Utc::now(),
