...
pub fn is_crypto_symbol(symbol: &str) -> bool {
    matches!(
        ...
        "BTC" | "ETH" | "SOL" | "BNB" | "XRP" | "ADA" | "DOGE" | "AVAX"
            | "MATIC" | "LINK" | "DOT" | "ATOM" | "LTC" | "BCH" | "UNI"
            | "AAVE" | "NEAR" | "ICP" | "FIL" | "APT" | "ARB" | "OP"
            | "SUI" | "INJ" | "TIA" | "SEI" | "PEPE" | "WIF" | "SHIB"
            | "TON" | "TRX" | "XLM"
    )
}
...
pub async fn fetch_live_bar(
    ...
    let (_, high, low, volume, _) = fetch_price_stats(client, symbol)
        .await
        .unwrap_or((price, price * 1.02, price * 0.98, 0.0, 0.0));
...
async fn fetch_binance_24hr_stats(
    ...
    let last_price = resp["lastPrice"]
        .as_str()
        .and_then(|s| s.parse::<f64>().ok())
        .unwrap_or(0.0);
    let high = resp["highPrice"]
        .as_str()
        .and_then(|s| s.parse::<f64>().ok())
        .unwrap_or(0.0);
    let low = resp["lowPrice"]
        .as_str()
        .and_then(|s| s.parse::<f64>().ok())
        .unwrap_or(0.0);
    let volume = resp["volume"]
        ...
        .and_then(|s| s.parse::<f64>().ok())
        .unwrap_or(0.0);
    let change_pct = resp["priceChangePercent"]
        .as_str()
        .and_then(|s| s.parse::<f64>().ok())
        .unwrap_or(0.0);

    Ok((last_price, high, low, volume, change_pct))
}
...
pub async fn fetch_binance_klines(
    ...
    for kline in resp {
        ...
        let volume = kline[5]
            ...
            .and_then(|s| s.parse::<f64>().ok())
            .unwrap_or(0.0);
        let dt = chrono::DateTime::from_timestamp_millis(open_time)
...
pub async fn fetch_kraken_price(
    ...
    let pair_data = result
        ...
        .ok_or("no pair data")?;
    let price_str = pair_data["c"][0]
        .as_str()
        .ok_or("no close price")?;
    Ok(price_str.parse()?)
}
...
async fn fetch_yahoo_price_stats(
    ...
    let high = quotes["high"]
        .as_array()
        .and_then(|a| a.iter().filter_map(|v| v.as_f64()).max_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal)))
        .unwrap_or(price);
    let low = quotes["low"]
        .as_array()
        .and_then(|a| a.iter().filter_map(|v| v.as_f64()).min_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal)))
        .unwrap_or(price);
    let volume: f64 = quotes["volume"]
        .as_array()
        .map(|a| a.iter().filter_map(|v| v.as_f64().filter(|x| *x > 0.0)).sum())
        .unwrap_or(0.0);
    let change_pct = resp["chart"]["result"][0]["meta"]["chartPreviousClose"]
        .as_f64()
        .map(|prev| if prev > 0.0 { (price - prev) / prev * 100.0 } else { 0.0 })
        .unwrap_or(0.0);
