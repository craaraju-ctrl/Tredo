...
impl PolicyCache {
    ...
    pub fn new(state: SharedState) -> Self {
        Self {
            ...
            top_win_rate_history: parking_lot::RwLock::new(Vec::with_capacity(MAX_HIT_RATE_HISTORY)),
    ...
    pub async fn seed_from_history(&self) -> usize {
        ...
        for trade in &trades {
            ...
            let entry_time =
                chrono::DateTime::parse_from_rfc3339(&trade.entry_time)
    ...
    pub fn save(&self) {
        ...
        let total_lookups = self.cache_lookups.load(std::sync::atomic::Ordering::Relaxed);
            ...
            Ok(json) => match serde_json::from_str::<PolicyCacheDisk>(&json) {
                Ok(disk) => {
                    ...
                    *self.cache.write() = disk.entries;
                    self.cache_lookups.store(disk.total_lookups, std::sync::atomic::Ordering::Relaxed);
                    self.cache_hits.store(disk.total_hits, std::sync::atomic::Ordering::Relaxed);
                ...
                Err(_) => {
                    ...
                    if let Ok(loaded) = serde_json::from_str::<HashMap<u64, PolicyEntry>>(&json) {
                        ...
                        *self.cache.write() = loaded;
                        tracing::info!("PolicyCache loaded {} entries from disk (legacy format)", n);
                    } else {
    ...
    pub fn record_lookup(&self, was_hit: bool) {
        self.cache_lookups.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        if was_hit {
    ...
    pub fn hit_stats(&self) -> (u64, u64, f64) {
        let total = self.cache_lookups.load(std::sync::atomic::Ordering::Relaxed);
    ...
    fn record_streak_snapshot(&self) {
        let streak = self.current_streak.load(std::sync::atomic::Ordering::Relaxed) as f64;
...
fn compute_confluence_simple(symbol: &str, current_price: f64, bars: &[tredo_core::OhlcvBar]) -> f64 {
    use tredo_core::{calculate_confluence_score, calculate_pivot_points, MarketContext, PivotMethod};
