...
impl Default for LlmExecutor {
    fn default() -> Self {
        ...
        let model = std::env::var("LLM_MODEL").unwrap_or_else(|_| "ministral:3b".to_string());
        let endpoint =
...
"#,
            ...
            .await
        {
            ...
            Err(e) => {
                println!(
                    "[LlmExecutor] ⚠ Ollama request failed (network): {}. Defaulting to HOLD.",
                    e
                );
        ...
        if !status.is_success() {
            ...
            println!(
                "[LlmExecutor] ⚠ Ollama error {}: {}. Defaulting to HOLD.",
                status,
                err_text.trim()
            );
            return LlmTradeDecision::default();
        ...
        let ollama_res: OllamaResponse = match res.json().await {
            ...
            Err(e) => {
                println!(
                    "[LlmExecutor] ⚠ Failed to parse Ollama response: {}. Defaulting to HOLD.",
                    e
                );
                return LlmTradeDecision::default();
            }
...
SUMMARY:"#,
            ...
            .await
        {
            Ok(r) => r,
            Err(e) => {
                println!("[LlmExecutor] ⚠ News summarization failed (small local model or slow inference?): {e}");
                return format!(
                    "Fetched {} headlines, summarization unavailable (timeout on local LLM).",
                    headlines.len()
                );
            }
        };
