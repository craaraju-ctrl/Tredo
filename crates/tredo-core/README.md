...
- **Pattern Detection** — 27 candlestick pattern detectors with multi-timeframe confirmation
...
| `patterns.rs` | 27 candlestick pattern detectors + multi-timeframe confirmation |
| `advanced_patterns.rs` | H&S, double tops/bottoms, wedges, flags, channels |
| `options.rs` | Black-Scholes Greeks, options strategies, chain signals |
| `portfolio_analytics.rs` | Kelly criterion, mean-variance optimization, VaR |
| `live_calendar.rs` | Live economic calendar (FMP / Alpha Vantage / built-in) |
| `service_manager.rs` | LLM/Kronos/broker health checks + status board |
