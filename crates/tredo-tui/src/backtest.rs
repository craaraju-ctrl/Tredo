...
pub fn render_backtest(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let trades = cache.get("total_trades").and_then(|v| v.as_u64()).unwrap_or(0);
    let win_rate = cache.get("win_rate").and_then(|v| v.as_f64()).unwrap_or(0.0);
    let pnl = cache.get("total_pnl").and_then(|v| v.as_f64()).unwrap_or(0.0);
    let dd = cache.get("max_drawdown").and_then(|v| v.as_f64()).unwrap_or(0.0);
    let sharpe = cache.get("sharpe_ratio").and_then(|v| v.as_f64()).unwrap_or(0.0);
    ...
    // ── Summary Cards Row ───────────────────────────────────────────────────
    let pnl_color = if pnl >= 0.0 { THEME.positive } else { THEME.danger };
    let wr_color = if win_rate >= 0.55 { THEME.positive }
        else if win_rate >= 0.4 { THEME.warning }
        else { THEME.danger };
    let dd_color = if dd <= 0.05 { THEME.positive }
        else if dd <= 0.10 { THEME.warning }
        else { THEME.danger };
    let sharpe_color = if sharpe >= 1.0 { THEME.positive }
        else if sharpe >= 0.5 { THEME.warning }
        else { THEME.danger };
    ...
    render_backtest_card(f, summary_row[0], "Trades", &format!("{}", trades), THEME.info);
    render_backtest_card(f, summary_row[1], "Win Rate", &format!("{:.1}%", win_rate * 100.0), wr_color);
    render_backtest_card(f, summary_row[2], "Total P&L", &format!("₹{:.0}", pnl), pnl_color);
    render_backtest_card(f, summary_row[3], "Max DD", &format!("{:.1}%", dd * 100.0), dd_color);
    ...
    for decision in &decisions {
        ...
        let color = if is_win { THEME.positive }
            else if is_loss { THEME.danger }
            else if is_entry { THEME.warning }
            else { THEME.muted };
