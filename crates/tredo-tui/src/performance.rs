...
pub fn render_performance(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    render_summary_card(
        ...
        "Return",
        &format!("{:+.1}%", eq_return),
        if eq_return >= 0.0 { THEME.positive } else { THEME.danger },
    );
    render_summary_card(
        f,
        summary_row[2],
        "Total P&L",
        &format!("₹{:.0}", pnl_total),
        if pnl_total >= 0.0 { THEME.positive } else { THEME.danger },
    );
    render_summary_card(
        f,
        summary_row[3],
    ...
    let conf_label = if conf_history.len() >= 2 { "Avg Confidence Trend" } else { "Confidence History" };
    let hit_label = if hit_history.len() >= 2 { "Avg Hit Rate Trend" } else { "Hit Rate History" };

    render_large_sparkline(f, bottom_row[0], "P&L Trend", &pnl_history, THEME.positive, false);

    // Right side: show confidence if available, otherwise hit rate
    if conf_history.len() >= 2 {
        render_large_sparkline(f, bottom_row[1], conf_label, &conf_history, THEME.neutral, true);
    } else {
        render_large_sparkline(f, bottom_row[1], hit_label, &hit_history, THEME.neutral, true);
    }
}
