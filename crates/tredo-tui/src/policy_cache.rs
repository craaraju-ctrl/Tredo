...
pub(crate) fn build_sparkline_line(history: &[f64], label: &str, as_percentage: bool) -> Line<'static> {
    if history.len() < 2 {
...
pub fn render_policy_cache(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let title_color = if is_loading { Color::Yellow } else { Color::Cyan };
    ...
    if let Some(cache) = data {
        ...
        let sparkline_hr = build_sparkline_line(&hit_history, "Hit Rate Trend", true);

        // Build top-performers win-rate sparkline line
        let sparkline_wr = build_sparkline_line(&wr_history, "Top Perf WR Trend", true);
        ...
        let (timestamp_line, ts_color) = match app.policy_cache_loaded_at {
            Some(t) => {
                ...
                let color = if secs < 10 { Color::Green } else { Color::Yellow };
                let text = if secs < 60 {
