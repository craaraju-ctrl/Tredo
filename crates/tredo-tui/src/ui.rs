...
pub fn ui(f: &mut Frame, app: &mut AppState) {
    ...
    // ── Clean up expired service alerts ─────────────────────────────
    app.service_alerts
        .retain(|(_, triggered_at)| triggered_at.elapsed() < std::time::Duration::from_secs(10));
    ...
    let footer_text = if let Some(h) = &app.health {
        let kronos_status = if h.get("kronos").and_then(|v| v.as_bool()).unwrap_or(false) {
            "K"
        } else {
            "x"
        };
        let orch_status = if h
            .get("orchestrator")
        ...
        let llm_status = if h.get("llm").and_then(|v| v.as_bool()).unwrap_or(false) {
            "L"
        } else {
            "x"
        };
        let model_name = app.current_model.as_deref().unwrap_or("unknown");
        format!(
    ...
    let footer = Paragraph::new(Line::from(vec![Span::raw(footer_text), ws_indicator]))
        .style(Style::default().fg(THEME.muted))
        .block(Block::default().borders(Borders::TOP));
    f.render_widget(footer, chunks[4]);

    // ── Service Alert Overlay (flashing red bar at top of content) ─────────
    if !app.service_alerts.is_empty() {
        // Concatenate all active alerts into a single line
        let alert_text = app
            .service_alerts
            .iter()
        ...
        // Flashing effect: toggle visibility every 500ms
        let elapsed_ms = app.service_alerts[0].1.elapsed().as_millis();
        let visible = (elapsed_ms / 500).is_multiple_of(2u128);
        ...
        if visible {
            ...
            // Render at top of content area (right below tabs row)
            let alert_area = Rect {
                x: 1,
                y: chunks[2].y,
            ...
            f.render_widget(alert_bar, alert_area);
        }
    }
...
fn render_overlay(f: &mut Frame, area: Rect) {
    ...
    let help_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                "Toggle trade entry form (Positions)",
                Style::default().fg(THEME.highlight),
            ),
        ]),
        Line::from(vec![
...
pub fn render_button_bar(f: &mut Frame, area: Rect, app: &mut AppState) {
    ...
    if let Some(filler) = chunks.last() {
        let hint = if app.action_running.is_some() {
            Line::from(vec![
