...
pub fn render_positions(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let lines: Vec<Line> = if positions.is_empty() {
        ...
        let mut lines = vec![
            Line::from(vec![
                Span::styled("SYMBOL", Style::default().fg(THEME.brand).add_modifier(Modifier::BOLD)),
                Span::raw("   "),
                Span::styled("DIR", Style::default().fg(THEME.brand).add_modifier(Modifier::BOLD)),
                Span::raw("   "),
                Span::styled("ENTRY", Style::default().fg(THEME.brand).add_modifier(Modifier::BOLD)),
                Span::raw("   "),
                Span::styled("CURRENT", Style::default().fg(THEME.brand).add_modifier(Modifier::BOLD)),
                Span::raw("   "),
                Span::styled("P&L", Style::default().fg(THEME.brand).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
        ];
        ...
        for p in positions.iter() {
            ...
            let current = p.get("current_price").and_then(|v| v.as_f64()).unwrap_or(entry);
            let pnl_pct = if entry > 0.0 { (current - entry) / entry * 100.0 } else { 0.0 };
            let pnl_color = if pnl_pct >= 0.0 { THEME.positive } else { THEME.negative };

            lines.push(Line::from(vec![
                Span::styled(sym.to_string(), Style::default().fg(THEME.highlight)),
                Span::raw("   "),
                Span::styled(dir.to_string(), Style::default().fg(if dir == "Long" { THEME.positive } else { THEME.negative })),
                Span::raw("   "),
                Span::styled(format!("{:.2}", entry), Style::default().fg(THEME.highlight)),
                Span::raw("   "),
                Span::styled(format!("{:.2}", current), Style::default().fg(THEME.highlight)),
                Span::raw("   "),
...
fn render_trade_form(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let fields: [(String, String); TRADE_FIELDS] = [
        ...
        ("Stop Loss".to_string(), format!("{:.2}", app.trade_entry_sl)),
        ("Take Profit".to_string(), format!("{:.2}", app.trade_entry_tp)),
    ];
    ...
    for (i, (label, value)) in fields.iter().enumerate() {
        ...
        let line = Line::from(vec![
            ...
            Span::styled(
                format!("{:<12}", label),
                Style::default().fg(label_fg).add_modifier(if focused { Modifier::BOLD } else { Modifier::default() }),
            ),
            Span::styled(
                value,
                Style::default().fg(fg).add_modifier(if focused { Modifier::BOLD } else { Modifier::default() }),
            ),
            Span::raw("  "),
    ...
    let hint = match app.trade_entry_focus {
        0 => Line::from(vec![
            ...
            Span::styled("↑ ↓", Style::default().fg(THEME.neutral).add_modifier(Modifier::BOLD)),
            Span::styled(" cycle symbol in watchlist  |  ", Style::default().fg(THEME.muted)),
            Span::styled("Enter", Style::default().fg(THEME.positive).add_modifier(Modifier::BOLD)),
        ...
        2 | 3 | 4 => {
            let step = if app.trade_entry_price > 1000.0 { 10.0 } else if app.trade_entry_price > 100.0 { 1.0 } else if app.trade_entry_price > 10.0 { 0.5 } else { 0.05 };
            Line::from(vec![
                Span::styled("  ← → switch field  |  ", Style::default().fg(THEME.muted)),
                Span::styled("↑ ↓", Style::default().fg(THEME.neutral).add_modifier(Modifier::BOLD)),
                Span::styled(format!(" adjust by {:.2}  |  ", step), Style::default().fg(THEME.muted)),
                Span::styled("Enter", Style::default().fg(THEME.positive).add_modifier(Modifier::BOLD)),
                Span::styled(" submit  ", Style::default().fg(THEME.muted)),
                Span::styled("Esc", Style::default().fg(THEME.negative)),
                Span::styled(" cancel", Style::default().fg(THEME.muted)),
