...
//! ## Usage
//! ```rust,ignore
//! // ZerodhaKiteBroker::new(api_key, api_secret, base_url, request_token)
...
impl ZerodhaKiteBroker {
    ...
    async fn get<T: serde::de::DeserializeOwned>(
        ...
        envelope.data.ok_or_else(|| KiteError::MissingField("data".into()))
    }
    ...
    fn kite_positions_to_tredo(
        ...
        for pos in positions {
            ...
            result.push(Position {                    id: format!(
                        "ZERODHA--{}--{}",
                        symbol,
...
impl BrokerAdapter for ZerodhaKiteBroker {
    ...
    async fn place_order(
        ...
        let mut form = vec![
            ...
            ("transaction_type", Self::kite_transaction_type(request.direction).to_string()),
            ("order_type", Self::kite_order_type(request.order_type).to_string()),
        ...
        let resp = self
            .client
            .post(&url)
            .header("Authorization", &self.auth_header().await.map_err(|e| e.to_string())?)
            .header("X-Kite-Version", "3")
            .form(&form)
            .send()
            .await
        ...
        tracing::info!("📈 Zerodha order placed: {} {} qty={} id={}",
            Self::kite_transaction_type(request.direction),
            request.symbol, request.qty, order_id,
        );
    ...
    async fn get_order_status(&self, order_id: &str) -> Result<OrderStatus, String> {
        ...
        let order = orders
            .into_iter()
            .find(|o| {
                o.order_id.as_deref() == Some(order_id)
                    || o.id.as_deref() == Some(order_id)
            })
    ...
    async fn get_recent_trades(&self, limit: usize) -> Result<Vec<ClosedTrade>, String> {
        ...
        let mut trades: Vec<ClosedTrade> = orders
            .into_iter()
            .filter(|o| {
                o.status.as_deref() == Some("COMPLETE")
                    || o.status.as_deref() == Some("FILLED")
            })
            .take(limit)
            .map(|o| {
...
mod tests {
    ...
    fn test_checksum_computation() {
        ...
        let checksum = ZerodhaKiteBroker::compute_checksum(
            "api_key",
            "api_secret",
            "request_token",
        );
        assert_eq!(checksum.len(), 64, "SHA-256 hex should be 64 chars");
        assert!(checksum.chars().all(|c| c.is_ascii_hexdigit()), "Should be hex");
    }
    ...
    fn test_parse_order_status() {
        ...
        assert_eq!(
            ...
            OrderStatus::Rejected { reason: "Order rejected by exchange".into() }
        );
        assert_eq!(
    ...
    fn test_kite_transaction_type() {
        assert_eq!(ZerodhaKiteBroker::kite_transaction_type(TradeDirection::Long), "BUY");
        assert_eq!(ZerodhaKiteBroker::kite_transaction_type(TradeDirection::Short), "SELL");
    }

    #[test]
    fn test_kite_order_type() {
        assert_eq!(ZerodhaKiteBroker::kite_order_type(OrderType::Market), "MARKET");
        assert_eq!(ZerodhaKiteBroker::kite_order_type(OrderType::Limit), "LIMIT");
        assert_eq!(ZerodhaKiteBroker::kite_order_type(OrderType::StopLoss), "SL");
        assert_eq!(ZerodhaKiteBroker::kite_order_type(OrderType::StopLossLimit), "SL-M");
    }
