...
async fn setup(db_name: &str) -> (AutonomousOrchestrator, String) {
...
async fn test_gate_blocks_prevents_downstream() {
    ...
    assert!(
        !summary.executed,
        "Pipeline should NOT execute when gate blocks"
    );
    assert!(
    ...
    println!("  ✅ Gate correctly blocked: \"{}\"", summary.reason);
...
async fn test_gate_drawdown_blocks_before_identifier() {
    let (orch, db_path) = setup("gate_dd").await;
    ...
    assert!(
        !summary.executed,
        "Should not execute when drawdown exceeds limit"
    );
...
async fn test_high_priority_overrides_medium() {
    let (orch, db_path) = setup("high_over_med").await;
...
async fn test_deb_layer_with_new_indicators() {
    let (orch, db_path) = setup("debate_indicators").await;
...
async fn test_judge_vetoes_low_confidence() {
    let (orch, db_path) = setup("judge_veto").await;
    ...
    *orch.state.market_regime.write().await =
    ...
    if verdict.judge_veto {
        assert_eq!(verdict.action, "HOLD", "Judge veto should override to HOLD");
        assert!(
            signal_opt.is_none(),
...
async fn test_low_priority_warnings_dont_block() {
    let (orch, db_path) = setup("low_warn").await;
...
async fn test_cot_chain_captures_all_layers() {
    ...
    *orch.state.market_regime.write().await =
    ...
    let has_hard_rules = chain_entries
        .iter()
        .any(|e| e.agent.contains("HardRules") || e.agent.contains("Gate"));
    let has_identifier = chain_entries.iter().any(|e| e.agent.contains("Identifier"));
    let has_debate = chain_entries
        .iter()
        .any(|e| e.agent.contains("Debate") || e.agent.contains("DebateLayer"));
...
async fn test_pipeline_resilience_varied_conditions() {
    let (orch, db_path) = setup("resilience").await;
...
async fn test_identifier_feeds_debate_context() {
    let (orch, db_path) = setup("ident_to_debate").await;
    ...
    *orch.state.market_regime.write().await =
    ...
    let ident_result = orch.tredo().run_identifier("SOL", 180.0, 1).await;
    assert!(
        ident_result.is_ok(),
    ...
    assert!(verdict.confidence > 0.0, "Confidence should be positive");
...
async fn test_medium_only_block_stops_pipeline() {
    ...
    // Bear regime + low confluence (0.25 < 0.30 bear minimum)
    // Seed 101 bars to bypass bootstrap override, and seed low conviction
    {
        let bar = OhlcvBar {
    ...
    *orch.state.market_regime.write().await =
    ...
    assert!(
        !gate_result.passed,
        "Medium should block without Higher override"
    );
    assert_eq!(
        gate_result.highest_failed_priority,
    ...
    assert!(result.is_ok());
    assert!(
        !result.unwrap().executed,
        "Pipeline should not execute on Medium block"
    );

    println!("  ✅ Medium block correctly stopped pipeline (bear regime + low confluence)");
