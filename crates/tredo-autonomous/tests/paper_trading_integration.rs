...
use tredo_autonomous::{
    types::TradeSignal,
    AutonomousOrchestrator, SharedState,
};
...
async fn test_paper_trade_full_cycle() {
    ...
    // 1. Create a trade signal: BUY 1 BTC at 65000, SL 64000, TP 67000
    ...
    assert!(result.is_ok(), "Paper trade should succeed: {:?}", result.err());
    ...
    //    Note: execution coordinator applies slippage (0.05%) to entry/SL/TP
    {
        ...
        assert_eq!(portfolio.open_positions.len(), 1, "Should have 1 open position");
        ...
        assert!(pos.stop_loss >= 64_000.0 && pos.stop_loss < 64_100.0, "SL should be near 64000 (slippage-adjusted), got {}", pos.stop_loss);
        assert!(pos.take_profit >= 67_000.0 && pos.take_profit < 67_100.0, "TP should be near 67000 (slippage-adjusted), got {}", pos.take_profit);
        println!(
    ...
    assert!(exec_result.is_ok(), "Execution coordinator run should succeed");
    ...
    assert_eq!(pos_count, 1, "Position should still be open (price not at SL/TP)");
...
async fn test_sl_tp_monitoring_triggers_stop_loss() {
    ...
    // Open a position: BUY 10 ETH at 3500, SL 3400, TP 3700
    let signal = make_signal(
        "ETH",
        TradeDirection::Long,
        3_500.0,
    ...
    // Simulate price drop to SL by updating position's current_price
    // Set well below SL to ensure trigger regardless of slippage direction
    {
...
async fn test_rejected_signal_zero_position_size() {
    ...
    let signal = make_signal("BTC", TradeDirection::Long, 65_000.0, 64_000.0, 67_000.0, 0.0);
    let result = orch.execution.execute_paper_trade(&signal).await;

    assert!(result.is_err(), "Signal with position_size=0 should be rejected");
    let err = format!("{}", result.unwrap_err());
    assert!(err.contains("Invalid position size"), "Error should mention invalid position size: {}", err);
...
async fn test_sl_tp_monitoring_triggers_take_profit() {
    ...
    // Open a position: BUY 50 SOL at 180, SL 175, TP 195
    let signal = make_signal(
        "SOL",
        TradeDirection::Long,
        180.0,
    ...
    //    Note: TP is adjusted by slippage (~0.05%), so we set price slightly above
    {
        let mut portfolio = orch.state.portfolio.write().await;
        let pos = portfolio
            .open_positions
            ...
            .find(|p| p.symbol == "SOL")
            .expect("Position should exist");
        // Set price well above the adjusted TP to ensure trigger
...
async fn test_multiple_positions_partial_close() {
    ...
    // Open two positions
    let btc_signal = make_signal("BTC", TradeDirection::Long, 65_000.0, 64_000.0, 67_000.0, 1.0);
    let eth_signal = make_signal("ETH", TradeDirection::Long, 3_500.0, 3_400.0, 3_700.0, 5.0);

    orch.execution.execute_paper_trade(&btc_signal).await.unwrap();
    orch.execution.execute_paper_trade(&eth_signal).await.unwrap();

    assert_eq!(orch.state.portfolio.read().await.open_positions.len(), 2);

    // Trigger SL on ETH only
    {
        let mut portfolio = orch.state.portfolio.write().await;
        let eth_pos = portfolio
            .open_positions
            .iter_mut()
            .find(|p| p.symbol == "ETH")
            .unwrap();
        eth_pos.current_price = 3_400.0; // Hit SL
    }
...
async fn test_pipeline_then_execution() {
    ...
    assert!(conf >= 0.0 && conf <= 1.0, "Confluence should be 0-1");
    println!(
    ...
    let signal = make_signal("NIFTY", TradeDirection::Long, 24_500.0, 24_300.0, 24_900.0, 1.0);
