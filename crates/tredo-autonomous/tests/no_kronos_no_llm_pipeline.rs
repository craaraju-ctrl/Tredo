...
fn setup_no_external_deps(db_name: &str) -> (AutonomousOrchestrator, String) {
...
async fn seed_aggregated_signal(state: &SharedState, symbol: &str) {
    ...
    *last_agg = Some(agg);
    println!("  📊 Seeded aggregated signal for {} with conviction=0.75", symbol);
}
...
async fn test_full_pipeline_no_kronos_no_llm() {
    let (orch, db_path) = setup_no_external_deps("pipeline_noext");
...
async fn test_identifier_with_kronos_down() {
    let (orch, db_path) = setup_no_external_deps("ident_kronos_down");
    seed_rich_ohlcv(&orch.state, "ETH", 3_500.0).await;
...
async fn test_strategy_decision_no_external_deps() {
    let (orch, db_path) = setup_no_external_deps("strat_noext");
    seed_rich_ohlcv(&orch.state, "BTC", 65_000.0).await;
    ...
    match signal.unwrap() {
        Some(sig) => {
            ...
            println!("  📝 Reasoning: {}", sig.reasoning.chars().take(120).collect::<String>());
        }
        None => {
...
async fn test_debate_agents_without_llm() {
    let (orch, db_path) = setup_no_external_deps("debate_noext");
    ...
    assert!(
        !reason.is_empty(),
        "Debate reasoning should not be empty"
    );
    assert_eq!(turns.len(), 4, "Debate should have exactly 4 turns (prop/crit/risk/hist)");

    // Each turn should have a valid action
    for turn in &turns {
        assert!(
            !turn.action.is_empty(),
            "Turn action should not be empty"
        );
        assert!(
...
async fn test_multiple_iterations_no_degradation() {
    let (orch, db_path) = setup_no_external_deps("multi_iter");
    seed_rich_ohlcv(&orch.state, "BTC", 65_000.0).await;
...
async fn test_all_16_agents_execute() {
    let (orch, db_path) = setup_no_external_deps("all_agents");
    seed_rich_ohlcv(&orch.state, "BTC", 65_000.0).await;
    seed_rich_ohlcv(&orch.state, "ETH", 3_500.0).await;
    ...
    let verifier_result = orch
        .tredo()
        .run_verifier("BTC", 65_000.0, equity, 2)
        .await;
    assert!(
    ...
    println!("  📊 COT entries: {} (agents recording pipeline progress)", cot_count);
