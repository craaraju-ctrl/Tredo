...
impl EvidenceBuilder {
    ...
    pub fn net_score(&self) -> f64 {
        ...
        self.evidences.iter().map(|e| e.score * e.weight).sum::<f64>() / total_weight
    }
    ...
    pub fn summary(&self) -> String {
        ...
        format!("net={:.3} bull={} bear={} neutral={}", self.net_score(), bull, bear, neu)
    }
}
...
impl ProposerAgent {
    ...
    pub async fn propose(&self, input: &AgentInput) -> DebateTurn {
        ...
        evidence.add(
            &format!("patterns={}", patterns_count),
            pattern_score,
            0.10,
        );
        ...
        let memory_score = if trained_recall.contains("high regret") || trained_recall.contains("regret") {
...
impl CriticAgent {
    ...
    pub async fn critique(&self, proposal: &str, input: &AgentInput) -> DebateTurn {
        ...
        if bars.len() >= 10 {
            ...
            let older_vol: f64 = bars.iter().rev().skip(5).take(5).map(|b| b.volume).sum::<f64>() / 5.0;
            ...
            if older_vol > 0.0 {
                ...
                if price_up && vol_ratio < 0.7 {
                    ...
                    concerns.push(format!("price rising on declining volume (vol ratio {:.2})", vol_ratio));
                }
                // Bullish: price rising with volume confirmation
                    ...
                    validations.push(format!("volume confirms price move (ratio {:.2})", vol_ratio));
                }
...
impl RiskAgent {
    ...
    pub async fn assess(&self, input: &AgentInput) -> DebateTurn {
        ...
        if portfolio_heat > 0.08 {
            evidence.add(&format!("high_heat={:.1}%", portfolio_heat * 100.0), -0.4, 0.20);
            risk_factors.push(format!("portfolio heat at {:.1}% — near safety limit", portfolio_heat * 100.0));
        } else if portfolio_heat > 0.05 {
            evidence.add(&format!("moderate_heat={:.1}%", portfolio_heat * 100.0), -0.15, 0.15);
        } else {
...
pub fn execute_conservative_degradation_policy(missing_skills_count: usize) -> DebateOutcome {
    if missing_skills_count > 2 {
        return DebateOutcome {
            ...
            consensus_justification: "CRITICAL DATA DEGRADATION: More than 2 skills failed. Trading paused.".to_string(),
        };
    }
