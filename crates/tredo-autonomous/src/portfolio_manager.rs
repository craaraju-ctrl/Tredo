...
impl PortfolioManagerAgent {
    ...
    pub async fn add_position(
        ...
        // ═══ 1/25 CAPITAL ALLOCATION RULE (Fix #3: cash-aware) ═════════
        // Each symbol can consume at most 4% (1/25) of total equity.
        // HOWEVER, the effective cap must also respect available cash.
        // Using only total_equity * 0.04 is wrong when cash is already
        // deployed — you'd think you can trade when you can't.
        // Fix: max_per_symbol = min(4% of equity, 95% of cash_balance).
        let total_equity = portfolio.total_equity.max(1.0);
        let cash_available = portfolio.cash_balance.max(1.0);
        let max_from_equity = total_equity * 0.04;
        let max_from_cash = cash_available * 0.95;
        let max_per_symbol = max_from_equity.min(max_from_cash);
        let position_value = signal.position_size * signal.entry_price;
        if position_value > max_per_symbol {
            return Err(format!(
        ...
        let now = Utc::now();
        portfolio.last_trade_time = Some(now);
