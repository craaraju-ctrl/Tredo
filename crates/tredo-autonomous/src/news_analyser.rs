    ...
    pub async fn analyze_and_store(&self, symbol: &str) -> f64 {
        ...
        if headlines.is_empty() {
            ...
            let ctx = tredo_core::NewsContext {
                ...
                headlines: vec![],
                summary: "No recent multi-API news snapshot. Neutral (awaiting loop fetch)."
                    .to_string(),
        ...
        let joined = headlines
            ...
            .join(" ");
        let pos_words = [
            "bull", "surge", "gain", "up", "rise", "positive", "beat", "strong", "adopt", "buy",
            "rally", "high", "adoption",
        ];
        let neg_words = [
            "bear", "drop", "loss", "down", "fall", "negative", "miss", "weak", "sell", "crash",
            "low", "risk", "warning",
        ];
        let mut pos = 0.0_f64;
        let mut neg = 0.0_f64;
        ...
        if joined.contains("finnhub")
            || joined.contains("marketaux")
            || joined.contains("reuters")
            || joined.contains("bloomberg")
            || joined.contains("coingecko")
        {
            pos += 0.6;
        }
        ...
        let summary = format!(
            ...
            headlines
                .first()
                .map(|h| h.title.chars().take(70).collect::<String>())
                .unwrap_or_default()
        );
