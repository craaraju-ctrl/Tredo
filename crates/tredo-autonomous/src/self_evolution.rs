...
impl SelfEvolutionValidator {
    ...
    pub async fn run_extended_validation(
        ...
        // Set env vars for induced regret
        if induce_regret {
            std::env::set_var("VALIDATION_INDUCE_REGRET", "true");
            std::env::set_var("VALIDATION_TIGHT_SL_PCT", "0.5");
            println!(">>> INDUCING REGRET: 0.5% tight SL for all trades");
        } else {
            std::env::remove_var("VALIDATION_INDUCE_REGRET");
        }
