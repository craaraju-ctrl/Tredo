...
impl OnChainData {
    ...
    pub async fn fetch_onchain(&self, symbol: &str) -> f64 {
        ...
        if symbol == "BTC" {
            if let Ok(score) = self.fetch_blockchain_com_signals().await {
                ...
                println!("[OnChain] {} API score from blockchain.com: {:.2}", symbol, score);
            }
        }
        ...
        if api_score.is_none() {
            if let Ok((score, note)) = self.fetch_coingecko_onchain(symbol).await {
                ...
                println!("[OnChain] {} CoinGecko API call failed — falling back to local proxy", symbol);
            }
        }
    ...
    async fn fetch_coingecko_onchain(&self, symbol: &str) -> Result<(f64, String), Box<dyn Error + Send + Sync>> {
        let id = match symbol {
            "BTC" => "bitcoin",
            "ETH" => "ethereum",
            "SOL" => "solana",
        ...
        let vol_24h = md.get("total_volume")
            .and_then(|x| x.get("usd"))
            .and_then(|x| x.as_f64())
            .unwrap_or(0.0);
        let price_chg_24h = md.get("price_change_percentage_24h")
            .and_then(|x| x.as_f64())
            .unwrap_or(0.0);
        let mcap = md.get("market_cap")
            .and_then(|x| x.get("usd"))
            .and_then(|x| x.as_f64())
            .unwrap_or(0.0);
        ...
        println!(
            ...
            symbol, vol_24h / 1e9, price_chg_24h, mcap / 1e9
        );
        ...
        let note = format!("CoinGecko vol=${:.1}B chg={:+.1}%", vol_24h / 1e9, price_chg_24h);
        Ok((score, note))
    }
