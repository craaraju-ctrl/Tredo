...
impl DebateLayer {
    ...
    pub async fn run_debate(
        ...
        self.run_debate_with_confluence(symbol, current_price, None)
            .await
    }

    /// Run debate with optional identifier-layer confluence (from MI, post-L2).
    pub async fn run_debate_with_confluence(
        &self,
        symbol: &str,
        current_price: f64,
        ...
        println!(
            "\n╔══ DEBATE LAYER ══╗ Symbol: {} @ {:.2}",
            symbol, current_price
        );

        // ── Assemble Intelligence ──────────────────────────────────────────
        let ctx = self
            .assemble_context(symbol, current_price, identifier_confluence)
            .await;
        println!(
        ...
        println!(
            "[Debate] Reasoning: {}",
            &verdict.reasoning[..verdict.reasoning.len().min(200)]
        );
        println!("╚════════════════════╝");

        // ── Build Signal from Verdict ──────────────────────────────────────
        let signal = if !verdict.judge_veto && (verdict.action == "BUY" || verdict.action == "SELL")
        {
    ...
    async fn assemble_context(
        ...
        let mut confluence = crate::helpers::resolve_symbol_confluence(&self.state, symbol).await;
        if let Some(mi_conf) = identifier_confluence {
            confluence = mi_conf.max(confluence);
        }
    ...
    async fn round1_bull_propose(&self, ctx: &DebateContext) -> DebateRoundOutput {
        ...
        let rsi_score = if ctx.rsi < 30.0 {
            0.6
        } else if ctx.rsi > 70.0 {
            -0.5
        } else {
            (50.0 - ctx.rsi) / 100.0
        };
        ...
        // 5b. Identifier/MI confluence (feeds debate with L2 intelligence)
        if ctx.confluence >= 0.55 {
            evidence.add(
                &format!("mi_confluence={:.0}%", ctx.confluence * 100.0),
                (ctx.confluence - 0.5) * 1.2,
                0.20,
            );
        }
        ...
        let buy_threshold = if ctx.confluence >= 0.60 { 0.12 } else { 0.18 };
        let action = if (net > buy_threshold && bull >= bear)
            || (ctx.confluence >= 0.65 && ctx.regime == MarketRegime::TrendingBull && bull >= bear)
        {
            "BUY"
        } else {
            "HOLD"
        ...
        DebateRoundOutput {
            ...
            evidence,
            reasoning: format!(
                "Bull case: {} bullish signals, {} bearish. Net={:.3}. {}",
                bull,
                bear,
                net,
                if action == "BUY" {
                    "Entry justified by regime + skill consensus + momentum."
                } else {
                    "Insufficient bullish evidence."
                }
            ),
            key_challenges: vec![
    ...
    async fn round1_bear_propose(&self, ctx: &DebateContext) -> DebateRoundOutput {
        ...
        evidence.add(
            &format!("regime_risk={}", ctx.regime_label),
            regime_score,
            0.25,
        );
        ...
        if ctx.portfolio_heat > 0.05 {
            evidence.add(
                &format!("heat={:.1}%", ctx.portfolio_heat * 100.0),
                -0.4,
                0.20,
            );
        ...
        DebateRoundOutput {
            ...
            evidence,
            reasoning: format!(
                "Bear case: {} bearish signals, {} bullish. Net={:.3}. {}",
                bear,
                bull,
                net,
                if action == "SELL" {
    ...
    async fn round2_adversarial(
            ...
            if ev.score.abs() > 0.2 {
                if ev.score > 0.0 && team == "Bear" {
                    ...
                    challenges.push(format!(
                        "Challenging {} (score {:.2}): context suggests this may be overconfident",
                        ev.factor, ev.score
                    ));
                } else if ev.score < 0.0 && team == "Bull" {
                    evidence.add(&format!("challenge_{}", ev.factor), -ev.score * 0.5, 0.3);
                    challenges.push(format!(
                        "Challenging {} (score {:.2}): risk is priced in by current levels",
                        ev.factor, ev.score
                    ));
                }
            }
        ...
        if team == "Bear" {
            if ctx.consecutive_losses >= 2 {
                ...
                challenges.push(format!(
                    "Bull team ignores {} consecutive losses",
            ...
            if ctx.portfolio_heat > 0.06 {
                ...
                challenges.push(format!(
                    "Bull team ignores portfolio heat at {:.1}%",
                    ctx.portfolio_heat * 100.0
                ));
            }
            ...
            if ctx.obv_direction > 0.0 && ctx.macd_hist < 0.0 {
                ...
                challenges.push(
                    "OBV shows bullish volume but MACD is negative — volume/price divergence"
                        .to_string(),
                );
            }
            ...
            if ctx.adx > 25.0 && ctx.minus_di > ctx.plus_di {
                ...
                challenges.push(format!(
                    "ADX {:.0} confirms downtrend (+DI {:.0} < -DI {:.0})",
                    ctx.adx, ctx.plus_di, ctx.minus_di
                ));
            }
            ...
            if ctx.vwap_deviation < -0.002 {
                ...
                challenges.push(format!(
                    "Price {:.1}% below VWAP — institutional selling pressure",
                    ctx.vwap_deviation * 100.0
                ));
            }
        } else {
            if let Some(ref agg) = ctx.aggregated_signal {
                if agg.is_bullish(None) && agg.conviction > 0.6 {
                    ...
                    challenges.push(
                        "Bear team ignores strong bullish skill consensus with high conviction"
                            .to_string(),
                    );
                }
            }
            if ctx.rsi < 35.0 {
                ...
                challenges.push(format!(
                    "Bear team ignores RSI at {:.0} (oversold)",
                    ctx.rsi
                ));
            }
            ...
            if ctx.obv_direction > 0.0 {
                ...
                challenges.push(format!(
                    "Bear team ignores bullish OBV trend (dir={:.3})",
                    ctx.obv_direction
                ));
            }
            ...
            if ctx.adx > 25.0 && ctx.plus_di > ctx.minus_di {
                ...
                challenges.push(format!(
                    "ADX {:.0} confirms uptrend (+DI {:.0} > -DI {:.0})",
                    ctx.adx, ctx.plus_di, ctx.minus_di
                ));
            }
            ...
            if ctx.cci < -100.0 {
                ...
                challenges.push(format!(
                    "CCI at {:.0} — deeply oversold, bounce likely",
                    ctx.cci
                ));
            }
            ...
            if ctx.williams_r < -80.0 {
                ...
                challenges.push(format!(
                    "Williams %R at {:.0} — oversold, bounce signal ignored",
                    ctx.williams_r
                ));
            }
        }
        ...
        let net = evidence.net_score();
        let action = if (team == "Bull" && net > 0.15) || (team == "Bear" && net < -0.15) {
            "COUNTER"
        } else {
            "WEAKENED"
        };
    ...
    async fn round3_synthesize(
        ...
        // Synthesis verdict — incorporate identifier confluence from L2
        let confluence_bias = (ctx.confluence - 0.5) * 0.35;
        let adjusted_net = net + confluence_bias;
        let action = if (adjusted_net > 0.08 && bull_evidence >= bear_evidence)
            || (ctx.confluence >= 0.62
                && bull_net > bear_net
                && ctx.regime == MarketRegime::TrendingBull)
        {
            "BUY"
        } else if adjusted_net < -0.12 && bear_evidence > bull_evidence {
            "SELL"
        } else {
            "HOLD"
        };

        let confidence = if action == "BUY" || action == "SELL" {
            (0.45 + adjusted_net.abs() * 0.35 + ctx.confluence * 0.15).min(0.92)
        } else {
    ...
    async fn judge_adjudicate(
        ...
        // High MI confluence from L2 can satisfy judge threshold even if debate confidence is moderate.
        let effective_confidence = if ctx.confluence >= 0.60 && synthesis.action == "BUY" {
            synthesis.confidence.max(ctx.confluence * 0.85)
        } else {
            synthesis.confidence
        };

        if (synthesis.action == "BUY" || synthesis.action == "SELL")
    ...
    async fn build_signal_from_verdict(
        ...
        let (entry, mut stop_loss, mut take_profit, _rule_rr) =
            crate::helpers::compute_autonomous_levels(
                ...
                ctx.atr_pct,
                &rules,
                ctx.aggregated_signal.as_ref(),
                Some(direction),
            );

        // Ensure SL/TP geometry matches verdict direction (prevents zero-size / inverted stops).
        let min_stop_dist = entry * 0.01;
        let stop_dist = (entry - stop_loss).abs();
        let levels_valid = match direction {
            tredo_core::TradeDirection::Long => {
                stop_loss < entry && take_profit > entry && stop_dist >= min_stop_dist * 0.1
            }
            tredo_core::TradeDirection::Short => {
                stop_loss > entry && take_profit < entry && stop_dist >= min_stop_dist * 0.1
            }
        ...
        if !levels_valid {
            stop_loss = if direction == tredo_core::TradeDirection::Long {
                entry - min_stop_dist
            } else {
                entry + min_stop_dist
            };
            take_profit = if direction == tredo_core::TradeDirection::Long {
                entry + min_stop_dist * 2.0
            } else {
                entry - min_stop_dist * 2.0
            };
        }
        ...
        let (equity, cash_available, effective_risk) = {
            ...
            let conf_mult = (verdict.confidence / 0.7).clamp(0.5, 1.2);
            let loss_mult = if ctx.consecutive_losses >= 3 {
                0.5
            } else if ctx.consecutive_losses >= 2 {
                0.7
            } else {
                1.0
            };
            let heat_mult = if ctx.portfolio_heat > 0.08 {
                0.5
            } else if ctx.portfolio_heat > 0.05 {
                0.7
            } else {
                1.0
            };
            ...
            (eq, cash, (rules.max_risk_per_trade * mult).max(0.003))
        };

        let position_size = crate::helpers::calculate_position_size_with_cash(
            equity,
            effective_risk,
            entry,
            stop_loss,
        ...
        let position_size = if position_size * entry > max_per_symbol {
            max_per_symbol / entry.max(0.0001)
        } else {
            position_size
        };
