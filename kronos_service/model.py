...
def _load_pipeline():
    ...
    try:
        ...
        print(f"[Kronos] Loading {MODEL_ID}...")
        _pipeline = ChronosBoltPipeline.from_pretrained(
            MODEL_ID,
            device_map="cpu",
            torch_dtype=torch.float32,
        )
        print(f"[Kronos] ✅ {MODEL_ID} loaded — real AI forecasting active")
        return _pipeline
    except Exception as e:
        _pipeline_error = str(e)
        print(f"[Kronos] ⚠ Could not load {MODEL_ID}: {e}")
        print("[Kronos] ↩ Falling back to drift model. Run: python3 download.py")
        return None
