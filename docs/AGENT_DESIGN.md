...
| **SuperIntelligence** 🧬 | Decision layer upgrade | Very Low | Cross-validation, conviction stacking, decision trace, memory context — deterministic, no LLM |
...
### SuperIntelligence Sub-Agents

| Sub-Agent | Input | Output | Algorithm |
|-----------|-------|--------|-----------|
| **CrossValidationEngine** | Skill results + Aggregated signal | CrossValidationReport | Validates each signal against ≥2 independent sources (5 pairs), detects conflicts, assigns conflict_score |
| **ConvictionStack** | Debate synthesis + memory + validation | ConvictionStack (8 factors) | Multi-factor conviction: directional + confidence + agreement + memory + risk + pattern + timeframe + synthesis. Regime-adaptive coefficients |
| **DecisionTrace** | Conviction + final action | DecisionTrace (ranked factors) | Ranks factors by contribution %, includes direction emoji, cross-validation status, weight |
| **MemoryContext** | Vector memory + symbol + price | MemoryContext (win rate + regret) | Structured memory recall from vector store with win rate calculation and high-regret flag |
