...
graph TB
    ...
    subgraph "6-LAYER PIPELINE [Medium Loop — 5m]"
        ...
        L3_5[Layer 3.5: SuperIntelligence<br/>Cross-validation + Conviction Stacking]
    ...
    L1 -->|PASS| L2
    L2 --> L3
    L3 -->|BullTeam 12f| L3_5
    L3 -->|BearTeam 11f| L3_5
    L3_5 -->|Validated| L4
    L4 -->|APPROVE| L5
    L5 -->|Trade| BR
...
flowchart TB
    ...
    subgraph "Layer 3.5: SuperIntelligence (NEW)"
        SI[SuperIntelligence<br/>Cross-Validation + Conviction Stacking<br/>Decision Trace + Memory Context]
    end
    ...
    BT -->|12 factors| SYN
    BR -->|11 factors| SYN
    SYN --> SI[SuperIntelligence<br/>Conviction Stack + Cross-Validation]
    SI --> JUDGE
    JUDGE -->|APPROVE| EXEC
...
### 5.2.5 SuperIntelligence Decision Layer (Layer 3.5)

> **New layer** between DebateLayer and Judge — cross-validates every signal before it reaches the final decision.
...
BUY / SELL / HOLD]
    ...
    SYN -->|skill results| CVE
    MEM -->|structured recall| CS
    CVE -->|conflicts resolved| CS
    CS -->|conviction score| DT
    DT -->|trace logged| FINAL
```
...
|-----------|------|---------|
| **CrossValidationEngine** | `super_intelligence.rs` | Validates each signal against ≥2 independent sources. 5 validation pairs: MMM↔RegimeDetector, SR↔VolumeProfile, OrderFlow↔FundingRate, Sentiment↔OnChain, Volatility↔Liquidity. Detects conflicts and penalizes disagreements |
| **ConvictionStack** | `super_intelligence.rs` | 8-factor conviction: Directional + Confidence + Agreement + Memory + Risk + Pattern + Timeframe + Synthesis. Regime-adaptive coefficients (e.g., TrendingBull: directional=0.30, LowLiquidity: risk=0.35) |
...
| Primary Signal | Secondary Source | Conflict Penalty | Rationale |
|---------------|------------------|-----------------|-----------|
| MarketMetricsMeter | RegimeDetector | -0.15 | Indicator ensemble vs trend direction agreement |
| SupportResistance | VolumeProfile | -0.10 | Price levels vs volume distribution consistency |
| OrderFlow | FundingRate | -0.12 | Buy/sell pressure vs market sentiment alignment |
| SentimentAnalyzer | OnChainData | -0.10 | News sentiment vs accumulation/distribution |
| VolatilityCalculator | Liquidity | -0.08 | Volatility regime vs market quality |

### Conviction Factors (8)

| Factor | Default Weight | TrendingBull | TrendingBear | Ranging | Volatile | LowLiquidity |
|--------|---------------|-------------|--------------|---------|----------|--------------|
| **Directional** | 0.25 | **0.30** | 0.15 | 0.20 | 0.12 | 0.10 |
| **Confidence** | 0.15 | 0.12 | 0.15 | **0.20** | 0.15 | 0.12 |
| **Agreement** | 0.14 | 0.10 | 0.15 | 0.15 | 0.12 | 0.10 |
| **Memory** | 0.14 | 0.10 | **0.20** | 0.15 | 0.12 | 0.10 |
| **Risk** | 0.14 | 0.10 | 0.15 | 0.10 | **0.25** | **0.35** |
| **Pattern** | 0.08 | **0.10** | 0.05 | **0.10** | 0.08 | 0.05 |
| **Timeframe** | 0.06 | 0.08 | 0.08 | 0.08 | 0.06 | 0.04 |
| **Synthesis** | 0.04 | 0.05 | 0.07 | 0.02 | **0.10** | **0.14** |
| **Total** | 1.00 | 1.00 | 1.00 | 1.00 | 1.00 | 1.00 |

### Decision Trace Output

Every BUY/SELL decision now includes a ranked factor list:
```
...
║ Conviction breakdown: Directional=28% | Confidence=22% | Agreement=15% | ...
╚══════════════════════════════════════╝
```
